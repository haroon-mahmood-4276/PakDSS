<div>
    <div class="text-center mb-4">
        <h3 class="address-title mb-2">{{ isset($address) ? 'Edit' : 'Add new' }} address</h3>
        <p class="text-muted address-subtitle">{{ isset($address) ? 'Edit' : 'Add new' }} address for delivery</p>
    </div>
            ...
            <div class="form-check custom-option custom-option-icon checked">
                <label class="form-check-label custom-option-content" for="radioHomeAddress">
                    ...
                    <input name="address_type" class="form-check-input" type="radio" value="home"
                        id="radioHomeAddress"
                        {{ isset($address) ? ($address->address_type === 'home' ? 'checked' : '') : 'checked' }}>
                </label>
            </div>
        </div>
        <div class="col-md mb-md-0 mb-3">
            <div class="form-check custom-option custom-option-icon">
                <label class="form-check-label custom-option-content" for="radioOfficeAddress">
                    ...
                    <input name="address_type" class="form-check-input" type="radio" value="office"
                        ...
                        {{ isset($address) && $address->address_type === 'office' ? 'checked' : '' }}>
                </label>
            </div>
        </div>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-12 col-md-6">
            ...
            <input type="text" class="form-control @error('first_name') is-invalid @enderror" id="first_name"
                placeholder="Ex. Asim" name="first_name"
                value="{{ isset($address) ? $address->first_name : old('first_name') }}" maxlength="50" />
            @error('first_name')
        ...
        <div class="col-12 col-md-6">
            ...
            <input type="text" class="form-control @error('last_name') is-invalid @enderror" id="last_name"
                placeholder="Ex. Mehmood" name="last_name"
                value="{{ isset($address) ? $address->last_name : old('last_name') }}" maxlength="50" />
            @error('last_name')
                <div class="invalid-feedback">{{ $message }}</div>
            @else
                <p class="m-0">
    ...
    <div class="row mb-3">
        <div class="col-12">
            <label class="form-label" style="font-size: 15px" for="address_1">Address Line 1 <span
                    class="text-danger">*</span></label>
            <input type="text" class="form-control @error('address_1') is-invalid @enderror" id="address_1"
                placeholder="Ex. 36-Lawrence Road" name="address_1"
                value="{{ isset($address) ? $address->address_1 : old('address_1') }}" maxlength="250" />
            @error('address_1')
                <div class="invalid-feedback">{{ $message }}</div>
            @else
                <p class="m-0">
    ...
    <div class="row mb-3">
        <div class="col-8">
            ...
            <input type="text" class="form-control @error('address_2') is-invalid @enderror" id="address_2"
                placeholder="Ex. Dr.Ziauddin Ahmed Road" name="address_2"
                value="{{ isset($address) ? $address->address_2 : old('address_2') }}" maxlength="250" />
            @error('address_2')
                <div class="invalid-feedback">{{ $message }}</div>
            @else
                <p class="m-0">
        ...
        <div class="col-4">
            ...
            <input type="text" class="form-control @error('nearest_landmark') is-invalid @enderror"
                ...
                value="{{ isset($address) ? $address->nearest_landmark : old('nearest_landmark') }}"
                maxlength="250" />
    ...
    <div class="row mb-3">
        <div class="col-12">
            ...
            <input type="text" class="form-control @error('mobile_no') is-invalid @enderror" id="mobile_no"
                placeholder="Ex. 0323456789" name="mobile_no"
                value="{{ isset($address) ? $address->mobile_no : old('mobile_no') }}" maxlength="250" />
            @error('mobile_no')
                <div class="invalid-feedback">{{ $message }}</div>
            @else
                <p class="m-0"><small class="text-muted">Enter phone number.</small></p>
            @enderror
        </div>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-6">
            <label class="form-label" style="font-size: 15px" for="country">Country <span
                    class="text-danger">*</span></label>
            <select class="select2-size-lg form-select @error('country') is-invalid @enderror" id="country"
                name="country">
                @if (isset($address))
                    <option value="{{ $address->country->id }}" selected="selected">{{ $address->country->name }}
                    </option>
                @endif
            </select>
            <p class="m-0">
                <small class="@error('country') text-danger @else text-muted @enderror">Select country.</small>
            </p>
        </div>
        <div class="col-6">
            <label class="form-label" style="font-size: 15px" for="state">State <span
                    class="text-danger">*</span></label>
            <select class="select2-size-lg form-select @error('state') is-invalid @enderror" id="state"
                name="state">
                @if (isset($address))
                    <option value="{{ $address->state->id }}" selected="selected">{{ $address->state->name }}
                    </option>
                @endif
            </select>
            <p class="m-0">
                <small class="@error('state') text-danger @else text-muted @enderror">Select state.</small>
            </p>
        </div>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-6">
            <label class="form-label" style="font-size: 15px" for="city">City <span
                    class="text-danger">*</span></label>
            <select class="select2-size-lg form-select @error('city') is-invalid @enderror" id="city"
                name="city">

                @if (isset($address))
                    <option value="{{ $address->city->id }}" selected="selected">{{ $address->city->name }}
                    </option>
                @endif
            </select>
            <p class="m-0">
                <small class="@error('city') text-danger @else text-muted @enderror">Select city.</small>
            </p>
        </div>
        <div class="col-12 col-md-6">
            <label class="form-label" for="zip_code">Zip Code</label>
            <input type="text" id="zip_code" name="zip_code" class="form-control"
                value="{{ isset($address) ? $address->zip_code : old('zip_code') }}" placeholder="99950">
        </div>
    </div>
        ...
        <div class="col-6">
            <label class="switch">
                ...
                <input type="checkbox" name="default_delivery_address" class="switch-input" value="1"
                    {{ (isset($address) ? $address->default_delivery_address : old('default_delivery_address')) == '1' ? 'checked' : null }}>
        ...
        <div class="col-6">
            <label class="switch">
                ...
                <input type="checkbox" name="default_billing_address" class="switch-input" value="1"
                    {{ (isset($address) ? $address->default_billing_address : old('default_billing_address')) == '1' ? 'checked' : null }}>
                <span class="switch-toggle-slider">
                    <span class="switch-on"></span>
                    <span class="switch-off"></span>
                </span>
